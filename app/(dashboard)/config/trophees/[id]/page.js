...
export default async function EditTypeTropheePage({ params }) {
  ...
  const { data: trophyType } = await supabase
    .from('trophy_types')
    .select('*')
    .eq('id', id)
    .single()

  if (!trophyType) notFound()
